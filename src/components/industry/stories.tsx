...
const IndustryStories: FunctionComponent<IndustryStoriesProps> = ({ className, CasesSectionSubTitle, CasesSectionTitle, CaseSectionDesc, cases }) => {
    return (<>
        <section id="growth" className={cn(className, "p-8 relative z-10")}>
            <div className="bg-linear-to-b from-emerald-900 to-emerald-700 rounded-3xl py-40 text-white">
                <div className="max-w-5xl mx-auto mb-10">
                    <span className="border-2 rounded-full font-medium py-1 px-3">
                        The Result
                    </span>
                    <h2 className="text-8xl mt-4 font-semibold">{CasesSectionTitle}</h2>
                    <div className="grid mt-20 grid-cols-2 gap-x-20">
                        <div>
                            <h3 className="opacity-75 text-2xl mb-5">{CasesSectionSubTitle}</h3>
                            {CaseSectionDesc && <p className="opacity-75 max-w-lg">
                                {CaseSectionDesc}
                            </p>}
                            <Button asChild className="mt-5 group text-black" variant={"outline"}>
