    ...
    return (<div className={cn("px-5 bg-linear-to-b from-purple-950 to-purple-800 text-white", className)}>
        <div className="max-w-6xl border-x border-dashed border-purple-500 py-40 mx-auto">
            <div className="max-w-5xl mx-auto grid grid-cols-2 gap-10">
                <div>
                    <h3 className="text-6xl font-medium leading-18">
                ...
                <div>
                    ...
                    <p className="text-purple-300 leading-6 max-w-2xl">
                        {data.overview}
                    </p>
                </div>
            </div>
            ...
            {solutions.map((item, i) => <div key={i} data-active={item.name === curent} className="border-x border-dashed border-purple-500 data-[active=true]:bg-purple-400/10 duration-300">
                <button onClick={() => setCurent(item.name)} className="grid text-left cursor-pointer grid-cols-2 p-8 hover:bg-purple-400/10 w-full">
                    <h4 className="text-4xl font-medium">
                        {item.name} <ChevronDown data-active={item.name === curent} className="inline data-[active=true]:-rotate-90 duration-500" size={36} />
                    </h4>
                    <p className="">
                        {item.desc}
                    </p>
                </button>
                <div data-active={item.name === curent} className="grid border-t border-dashed border-purple-500 grid-cols-3 h-0 mt-0 mb-0 opacity-0 data-[active=true]:opacity-100 duration-300 invisible data-[active=true]:visible data-[active=true]:h-auto divide-x divide-purple-500 divide-dashed">
                    {item.features?.map((m, im) => <div className="p-10" key={im}>
                        <div className="size-14 bg-purple-950 flex rounded-lg mb-5">
                            <Icon icon={m.icon} className="m-auto size-7 text-purple-300" strokeWidth={1} />
                        </div>
        ...
        <div className="max-w-6xl border-x border-dashed border-purple-500 p-2 mx-auto">
            <div className="grid grid-cols-2 gap-10 p-20 bg-purple-600 rounded">
                <div>
                    <CircleQuestionMarkIcon size={46} strokeWidth={1.5} />
                    <h3 className="text-2xl mb-2 mt-5">
